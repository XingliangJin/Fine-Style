...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.content_num = config.content_mlp_dim
...
# co_data, cl_data在data_loader.py里面
    def forward(self, co_data, cl_data, mode):
        ...
        if mode == 'gen_update':
            ...
            s_xao = self.gen.transformer(co_data[stylestr], la_content.to(self.device)) # xxx, "2d" / "3d"
            s_xbo = self.gen.transformer(cl_data[stylestr], la_content.to(self.device))
            s_xposo = self.gen.transformer(co_data[same_style], la_content.to(self.device))  # another motion with the same label


            # add content label to style code

            la_emb = self.gen.content_embedding(la_content.to(self.device))
            ...
            s_xpos = torch.mul(torch.sigmoid(c_la),torch.sigmoid(s_xposo))

            #res
            s_xa = s_xa + torch.sigmoid(s_xao)
            s_xb = s_xb + torch.sigmoid(s_xbo)
            s_xpos = s_xpos + torch.sigmoid(s_xposo)
            ...
            with torch.no_grad():
                ...
                la_content = co_data["content_label"]
                c_xa = self.gen.enc_content(co_data["content"])
                s_xao = self.gen.transformer(co_data[stylestr], la_content.to(self.device))
                s_xbo = self.gen.transformer(cl_data[stylestr], la_content.to(self.device))
    ...
    def test_rec(self, data):
        ...
        with torch.no_grad():
            la_content = data["content_label"]
            c_x = self.gen.enc_content(x)
            s_xo = self.gen.transformer(y, la_content.to(self.device))
            s_po = self.gen.transformer(yp, la_content.to(self.device))
            ...
            s_x = torch.mul(torch.sigmoid(c_la),torch.sigmoid(s_xo))
            s_p = torch.mul(torch.sigmoid(c_la),torch.sigmoid(s_po))

            #res
            s_x = s_x + torch.sigmoid(s_xo)
            s_p = s_p + torch.sigmoid(s_po)
    ...
    def test(self, co_data, cl_data, status):
        ...
        #["walk"0, "run"1, "jump"2, "kick"3, "punch"4, "trans"]
    ...
    def get_latent_codes(self, data):
        ...
        #return codes
    def get_style_code(self, cl_data):
        data = []
        label = []
        strs = ["walk","run","jump","kick","punch"]
        
        self.eval()
        self.gen.eval()
        yb = cl_data["style3d"]

        s_xb = self.gen.enc_style(yb, "3d")
        for i, str in zip(range(5), strs):
            content_label = i
            xxx = torch.tensor([content_label])
            la_emb = self.gen.content_embedding(xxx.to(self.device))
            c_la = self.gen.content_mlp(la_emb)
            c_la = c_la.unsqueeze(-1)
            
            s_xb = torch.mul(torch.sigmoid(c_la),s_xb)
            data.append(s_xb.squeeze(-1))
            label.append(str)
        label = np.array(label)
        data = torch.stack(data).squeeze(1).detach().cpu().numpy()
        return data, label


    def get_style_code_by_style_label(self, cl_data):
        data = []
        label = []
        strs = ["walk","run","jump","kick","punch"]
        #strs = ["style_walk_code"]
        ...
        #     data.append(s_xb.squeeze(-1))
        #     label.append(cl_data["label"])
        label = torch.stack(label).squeeze(0).detach().cpu().numpy()
        data = torch.stack(data).squeeze(0).detach().cpu().numpy()
        return data, label